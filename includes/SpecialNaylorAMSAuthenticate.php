...
class SpecialNaylorAMSAuthenticate extends UnlistedSpecialPage {

    // Borrowed from SpecialPluggableAuthLogin
    ...
    public function execute($param) {
        ...
        $authToken = $request->getVal('AuthenticationToken') ?? '';

        if ($authToken === '') {
            // Error
            $output->addWikiText('Hmm, that doesn\'t seem right. This page should not be visited by a person manually. [[Main_Page|Return to the main page]]');
            return;
        }

        // Since there is no better way to pass the auth token, we will pass it via the username param,
        // which will be overridden with the correct value after authenticate() is called.
        $username = $authToken;
        
        /*
         * The rest of this code is borrowed from SpecialPluggableAuthLogin in PluggableAuth extension
        */
